...
export function BookingTable({bookings: initialBookings}: BookingTableProps) {
    ...
    const disabledStatuses = ["pending", "completed", "cancelled", "overdue"]
                    ...
                    {bookings.map((booking) => (
                        <>
                            <TableRow key={booking.booking_id}>
                                ...
                                <TableCell
                                    className="text-right font-medium">{formatPrice(booking.total_price)}</TableCell>
                                <TableCell>
                                        ...
                                        <DialogContent className="sm:max-w-[425px]">
                                            ...
                                            <div className="grid gap-4 py-4">
                                                ...
                                                <div className="grid grid-cols-3 items-center gap-4">
                                                    ...
                                                    <span
                                                        className="col-span-2">{formatDate(booking.booking_date)}</span>
                                                </div>
                                                <div className="grid grid-cols-3 items-center gap-4">
                                                ...
                                                <div className="grid grid-cols-3 items-center gap-4">
                                                    ...
                                                    <span
                                                        className="col-span-2 font-medium">{formatPrice(booking.total_price)}</span>
                                                </div>
                                                <div className="grid grid-cols-3 items-center gap-4">
                                                ...
                                                </div>
                                                {
                                                    booking.courts.map((court, index) => (
                                                        <div className="grid grid-cols-5 items-center gap-4">
                                                            <span
                                                                className="col-start-2 font-medium text-sm">{court.court_name}</span>
                                                            <span className="col-span-2 font-medium text-sm">{court.start_time} - {court.end_time}</span>
                                                        </div>
                                                    ))
                                                }
                                            </div>
                                                ...
                                                {booking.courts.map((court, index) => (
                                                    <div key={index} className="bg-background p-3 rounded-md border">
                                                        <div className="text-sm">
                                                            <span
                                                                className="font-medium">Court Name:</span> {court.court_name}
                                                        </div>
                                                        <div className="text-sm">
                                                            <span
                                                                className="font-medium">Time:</span> {formatTime(court.start_time)} -{" "}
                                                            {formatTime(court.end_time)}
                                                        </div>
                                                        <div className="text-sm">
                                                            <span
                                                                className="font-medium">Date:</span> {formatDate(booking.booking_date)}
                                                        </div>
                                                        <div className="text-sm">
                                                            <span
                                                                className="font-medium">Price:</span> {formatPrice(court.price)}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
